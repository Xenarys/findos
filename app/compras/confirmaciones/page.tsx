...
export default function ConfirmacionesGlobalPage() {
  ...
  async function cargarTodo() {
    ...
    const { data } = await supabase
      ...
      .select('*, ordenes_compra(numero, entidades(razon_social))')
  ...
  const confirmacionesFiltradas = confirmaciones.filter(c => {
    ...
    const matchBusqueda = busqueda === '' ||
      ...
      (c.ordenes_compra?.numero || '').toLowerCase().includes(busqueda.toLowerCase()) ||
      (c.ordenes_compra?.entidades?.razon_social || '').toLowerCase().includes(busqueda.toLowerCase())
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-semibold text-gray-800">Confirmaciones de Compra</h1>
          <div className="flex items-center gap-3">
            <input
              type="text"
              placeholder="Buscar OC, confirmación o proveedor..."
              value={busqueda}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100">
                    ...
                    <th className="text-left py-3 text-gray-600 font-medium">OC</th>
                    <th className="text-left py-3 text-gray-600 font-medium">Proveedor</th>
                ...
                <tbody>
                  {confirmacionesFiltradas.map(conf => (
                    <tr key={conf.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                      ...
                      <td className="py-3 font-mono text-gray-700">{conf.ordenes_compra?.numero}</td>
                      <td className="py-3 text-gray-700">{conf.ordenes_compra?.entidades?.razon_social}</td>
                      ...
                      <td className="py-3 text-center">
                        <button
                          onClick={() => router.push(`/compras/${conf.orden_compra_id}/confirmaciones/${conf.id}`)}
