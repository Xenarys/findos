...
export default function ConfirmacionesListPage() {
  const { id } = useParams()
  ...
  async function cargarTodo() {
    ...
    const [ocData, confsData] = await Promise.all([
      supabase.from('ordenes_compra').select('numero').eq('id', id).single(),
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-3 mb-6">
          <button onClick={() => router.push(`/compras/${id}`)} className="text-gray-400 hover:text-gray-600 text-sm">← Volver a OC</button>
          <h1 className="text-2xl font-semibold text-gray-800">Confirmaciones</h1>
          <span className="text-gray-500 text-sm">({oc?.numero})</span>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-6">
          {confirmaciones.length === 0 ? (
            <div className="text-center py-8">
              ...
              <button onClick={() => router.push(`/compras/${id}/confirmar`)} className="text-blue-600 hover:text-blue-800 text-sm mt-2">
                Crear confirmación →
              </button>
            </div>
          ) : (
