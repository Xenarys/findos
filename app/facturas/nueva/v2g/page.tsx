...
export default function NuevaFacturaV2GPage() {
  ...
  async function cargarTodo() {
    setLoading(true)
    const [clts, bs, imps, conds] = await Promise.all([
      ...
      supabase.from('impuestos').select('id, codigo, nombre, porcentaje, tipo, flujo, cuenta_id, fecha_desde, fecha_hasta').eq('activo', true).eq('flujo', 'venta').order('codigo'),
  ...
  async function agregarItem() {
    ...
    if (bien.esquema_tributario_venta_id) {
      const esq = await cargarEsquema(bien.esquema_tributario_venta_id)
      const adic = esq.filter(ei => ei.impuestos?.flujo === 'venta' && ei.impuestos?.tipo !== 'iva')
  ...
  async function guardar(emitir: boolean) {
    ...
    const { data: facturaData, error } = await supabase.from('facturas').insert([{
      ...
      tipo: 'V2G', flujo: 'venta',
      entidad_id: clienteId, fecha, moneda, tasa_cambio: tasaCambio,
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-2xl font-semibold text-gray-800">Nueva Factura Venta Directa (V2G)</h1>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-6 mb-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <select value={clienteId} onChange={e => setClienteId(e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm">
