...
interface ConfirmacionItem {
  ...
  bienes_servicios?: { afecto_iva_compra: boolean; codigo: string }
}
...
export default function NuevaFacturaC1GPage() {
  ...
  async function cargarInicial() {
    const { data: provs } = await supabase.from('entidades').select('id, razon_social, rut').eq('tipo_proveedor', true).eq('activo', true).order('razon_social')
    if (provs) setProveedores(provs)
    const { data: imps } = await supabase.from('impuestos').select('id, codigo, nombre, porcentaje, tipo, flujo, cuenta_id').eq('activo', true).eq('flujo', 'compra').order('codigo')
  ...
  async function cargarConfirmaciones() {
    ...
    const { data: ocs } = await supabase.from('ordenes_compra').select('id, numero, moneda').eq('proveedor_id', proveedorId)
    ...
    if (confs) {
      const confsConDetalle = await Promise.all(confs.map(async (conf: any) => {
        const { data: items } = await supabase
          ...
          .select('descripcion, cantidad_confirmada, subtotal_bruto_conf, subtotal_neto_conf, ordenes_compra_items(bienes_servicios(afecto_iva_compra, codigo))')
          .eq('confirmacion_id', conf.id)

        const totalNeto = items?.reduce((sum, i) => sum + i.subtotal_neto_conf, 0) || 0

        // Calcular IVA propuesto sumando solo los ítems afectos
        let totalIva = 0
        if (items) {
          for (const item of items) {
            const afecto = (item as any).ordenes_compra_items?.bienes_servicios?.afecto_iva_compra
            if (afecto) {
              // Buscar IVA vigente para compra
  ...
  function agregarConfirmacion(conf: Confirmacion) {
    ...
    // Detectar monedas distintas a CLP y agregar tasas necesarias
    const monedasNecesarias = new Set(nuevas.map(c => c.moneda_oc).filter(m => m !== 'CLP'))
    setTasasMoneda(prev => {
      const nuevasTasas = [...prev]
      monedasNecesarias.forEach(m => {
        if (!nuevasTasas.some(t => t.moneda === m)) {
          nuevasTasas.push({ moneda: m, tasa: 0, tasaStr: '' })
        }
      })
  ...
  // Convertir monto de cualquier moneda a CLP
