...
export default function EditarPVPage() {
  const { id } = useParams()
  ...
  async function cargarTodo() {
    setLoading(true)
    const [cls, bs, cps, imps, conds, pvData, itemsData, condsPV, impsPV] = await Promise.all([
      ...
      supabase.from('pedidos_venta').select('*').eq('id', id).single(),
      supabase.from('pedidos_venta_items').select('*, bienes_servicios(id, codigo, descripcion, clasificacion, unidad, moneda, afecto_iva_venta, esquema_tributario_venta_id)').eq('pedido_venta_id', id).order('numero_item'),
      supabase.from('pv_condiciones').select('*, condiciones_precio(nombre, abreviatura, tipo, forma_calculo)').eq('pedido_venta_id', id),
      supabase.from('pv_impuestos').select('*, impuestos(codigo, nombre, porcentaje, tipo)').eq('pedido_venta_id', id),
    ])
    ...
    if (pvData.data) {
      setCabecera({
        cliente_id: pvData.data.cliente_id || '',
        fecha: pvData.data.fecha || '',
        condicion_pago_id: pvData.data.condicion_pago_id || '',
        moneda: pvData.data.moneda || 'CLP',
        observaciones: pvData.data.observaciones || ''
      })
    }

    if (itemsData.data) {
      const itemsConDetalle = itemsData.data.map((i: any) => {
        const impsItem = (impsPV.data || []).filter((imp: any) => imp.item_id === i.id)
        const condsItem = (condsPV.data || []).filter((c: any) => c.item_id === i.id)
        return {
          id: i.id,
    ...
    if (condsPV.data) {
      setCondsCabecera((condsPV.data as any[]).filter(c => !c.item_id).map(c => ({
        id: c.id,
        condicion_precio_id: c.condicion_precio_id,
        nombre: c.condiciones_precio?.nombre || '',
        abreviatura: c.condiciones_precio?.abreviatura || '',
    ...
    if (impsPV.data) {
      setImpsCabecera((impsPV.data as any[]).filter(i => !i.item_id).map(i => ({
        id: i.id,
  ...
  async function guardar() {
    ...
    const { error } = await supabase.from('pedidos_venta').update({
      cliente_id: cabecera.cliente_id, fecha: cabecera.fecha,
    ...
    if (itemsEliminados.length > 0) {
      await supabase.from('pedidos_venta_items').delete().in('id', itemsEliminados)
    }

    await supabase.from('pv_condiciones').delete().eq('pedido_venta_id', id as string).is('item_id', null)
    await supabase.from('pv_impuestos').delete().eq('pedido_venta_id', id as string).is('item_id', null)
    ...
    for (let idx = 0; idx < items.length; idx++) {
      ...
      if (item.id) {
        ...
        await supabase.from('pv_impuestos').delete().eq('item_id', item.id)
        await supabase.from('pv_condiciones').delete().eq('item_id', item.id)

        if (item.impuestos.length > 0) {
          await supabase.from('pv_impuestos').insert(item.impuestos.map(imp => ({
            pedido_venta_id: id, item_id: item.id,
        ...
        const { data: itemData } = await supabase.from('pedidos_venta_items').insert([{
          pedido_venta_id: id, bien_servicio_id: item.bien_servicio_id,
          descripcion: item.descripcion, cantidad: item.cantidad,
          precio_unitario: item.precio_unitario, subtotal: item.subtotal_bruto,
          numero_item: numeroItem,
        ...
        if (itemData && itemData[0]) {
          ...
          if (item.impuestos.length > 0) {
            await supabase.from('pv_impuestos').insert(item.impuestos.map(imp => ({
              pedido_venta_id: id, item_id: itemId, impuesto_id: imp.impuesto_id,
          ...
          if (item.condiciones.length > 0) {
            await supabase.from('pv_condiciones').insert(item.condiciones.map(c => ({
              pedido_venta_id: id, item_id: itemId, condicion_precio_id: c.condicion_precio_id,
    ...
    if (condsCabecera.length > 0) {
      await supabase.from('pv_condiciones').insert(condsCabecera.map(c => ({
        pedido_venta_id: id, item_id: null, condicion_precio_id: c.condicion_precio_id,
    ...
    if (impsCabecera.length > 0) {
      await supabase.from('pv_impuestos').insert(impsCabecera.map(i => ({
        pedido_venta_id: id, item_id: null, impuesto_id: i.impuesto_id,
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-2xl font-semibold text-gray-800">Editar Pedido de Venta</h1>
        </div>
        ...
        <div className="flex justify-end gap-3">
          <button onClick={() => router.push(`/ventas/${id}`)} className="px-4 py-2 text-sm border border-gray-200 rounded-lg text-gray-600">Cancelar</button>
