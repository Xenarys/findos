...
export default function DetallePVPage() {
  const { id } = useParams()
  const router = useRouter()

  const [pv, setPv] = useState<any>(null)
  const [items, setItems] = useState<ItemPV[]>([])
  ...
  async function cargarTodo() {
    ...
    const [pvData, itemsData, condsData, impsData, confsData] = await Promise.all([
      supabase.from('pedidos_venta').select('*, entidades(razon_social, rut)').eq('id', id).single(),
      supabase.from('pedidos_venta_items').select('*, bienes_servicios(codigo, unidad)').eq('pedido_venta_id', id).order('numero_item'),
      supabase.from('pv_condiciones').select('*, condiciones_precio(nombre, abreviatura, tipo)').eq('pedido_venta_id', id),
      supabase.from('pv_impuestos').select('*, impuestos(codigo, nombre, tipo, porcentaje)').eq('pedido_venta_id', id),
  ...
  async function emitirPV() {
    if (!window.confirm('¿Emitir este PV? No podrá ser editado nuevamente.')) return
    setEmitiendo(true)
    const { error } = await supabase.from('pedidos_venta').update({ estado: 'emitida' }).eq('id', id)
    if (error) { alert('Error: ' + error.message); setEmitiendo(false); return }
    alert('PV emitido exitosamente')
    cargarTodo()
    setEmitiendo(false)
  }

  async function anularPV() {
    if (!window.confirm('¿Anular este PV? Esta acción no se puede deshacer.')) return
    setAnulando(true)
    const { error } = await supabase.from('pedidos_venta').update({ estado: 'anulada', documento_abierto: false }).eq('id', id)
  ...
  if (!pv) return <div className="min-h-screen bg-gray-50 flex items-center justify-center text-gray-400">PV no encontrado</div>
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-2xl font-semibold text-gray-800">Pedido de Venta</h1>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-6 mb-4">
          <div className="grid grid-cols-2 gap-6 mb-6">
            <div>
              ...
              <p className="text-lg font-semibold text-gray-800">{pv.numero}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 mb-1">Estado</p>
              <span className={`text-sm px-2 py-1 rounded font-medium ${
                pv.estado === 'borrador' ? 'bg-gray-100 text-gray-700' :
                pv.estado === 'emitida' ? 'bg-blue-100 text-blue-700' :
                'bg-red-100 text-red-700'
              }`}>
                {pv.estado === 'borrador' ? 'Borrador' :
                 pv.estado === 'emitida' ? 'Emitida' : 'Anulada'}
              </span>
            </div>
            <div>
              <p className="text-xs text-gray-500 mb-1">Cliente</p>
              <p className="text-sm text-gray-800">{pv.entidades?.razon_social}</p>
              <p className="text-xs text-gray-500">{pv.entidades?.rut}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 mb-1">Fecha</p>
              <p className="text-sm text-gray-800">{new Date(pv.fecha).toLocaleDateString('es-CL')}</p>
            </div>
          </div>
          <div className="border-t border-gray-100 pt-4">
            <p className="text-xs text-gray-500 mb-1">Observaciones</p>
            <p className="text-sm text-gray-700">{pv.observaciones || '—'}</p>
          </div>
        </div>
    ...
    <span className="font-mono">{fmt(g.total)}</span>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-6 mb-4">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-sm font-semibold text-gray-600 uppercase tracking-wide">Confirmaciones de venta</h2>
            <div className="flex gap-2">
              {confirmaciones.length > 0 && (
                <button onClick={() => router.push(`/ventas/${id}/confirmaciones`)} className="text-xs bg-gray-600 text-white px-3 py-1.5 rounded hover:bg-gray-700">
                  Ver todas
                </button>
              )}
              {pv.estado === 'emitida' && pv.documento_abierto && (
                <button onClick={() => router.push(`/ventas/${id}/confirmar`)} className="text-xs bg-blue-600 text-white px-3 py-1.5 rounded hover:bg-blue-700">
                  + Nueva confirmación
                </button>
              )}
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100">
                    ...
                    <th className="text-left py-2 text-gray-600 font-medium">HES</th>
        ...
        <div className="flex justify-end gap-3">
          {pv.estado === 'borrador' && pv.documento_abierto && (
            <>
              <button onClick={() => router.push(`/ventas/${id}/editar`)} className="px-4 py-2 text-sm border border-gray-200 rounded-lg text-gray-600 hover:bg-gray-50">
                Editar
              </button>
              <button onClick={emitirPV} disabled={emitiendo} className="px-6 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">
                {emitiendo ? 'Emitiendo...' : 'Emitir PV'}
              </button>
              <button onClick={anularPV} disabled={anulando} className="px-6 py-2 text-sm bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-50">
                {anulando ? 'Anulando...' : 'Anular'}
              </button>
            </>
          )}
          {(pv.estado === 'emitida' || pv.estado === 'anulada') && (
            <span className="text-sm text-gray-500 px-4 py-2">PV {pv.estado}</span>
          )}
        </div>
